...
import { addCourse, getAdmin, loginAdmin, logoutAdmin, removeCourse } from './controller';
...
async function handleRemoveCourse(req: Request, res: Response) {
  try {
    const addCoursestatus = await removeCourse(req.body);
    res.status(200).json({
      message: addCoursestatus.message,
    });
  } catch (e) {
...
adminAuthRouter.post('/removeCourse', handleRemoveCourse);
